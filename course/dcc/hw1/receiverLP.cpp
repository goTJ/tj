...
int main(int argv, char *argc[])
{
	...
	// {{{ help page
	if(fileIdx == -1){
		printf("Usage: %s [-m|-a|-u] [-number] lppcm_filename\n", argc[0]);
	...
	// {{{ initialize LP
	int code;
	for(int i=0; i<P_SIZE+E_SIZE&&(code=input(codeBit, fin))!=-1; i++){
		short d=quan->decode((unsigned int)code);
		double ir=0.0;
		...
		if(ir > 10000.0)
			...
			ir = -10000;
////		printf("%.0lf %d %d ", ir, code, d);
		d += (short)ir;
////		printf("%d\n", d);
		fwrite(&d, sizeof(short), 1, fout);
		X.push_front(d);
